...
exports.login = (req, res) => {
  ...
  const passwordConfirmFailed =
    req.query["password-confirm-failed"] !== undefined;
...
exports.register = async (req, res) => {
  ...
  if (password == passwordConfirm) {
    ...
    if (checkExists.length != 0) {
      ...
      const data = {
        message: "Success",
        url: `${process.env.DOMAIN_NAME}/register-success?email=${email}&username=${username}`,
      };
      res.status(200).json(data);
    }
  } else {
...
exports.editAccount = async (req, res) => {
  try {
    req.session.passport.user = await userService.edit(
      res.locals.user._id,
      req.body
    );
  ...
  try {
    ...
    if (await userService.validPassword(password0, req.user)) {
      if (password1 === password2) {
        req.session.passport.user = await userService.changePassword(
          req.user._id,
          password1
        );
...
exports.resetPassword = async (req, res) => {
  const { username } = req.query;
  const activationString = req.query["activation-string"];
  const user = await userService.findByUsernameAndAS(
    username,
    activationString
  );
  if (user) {
